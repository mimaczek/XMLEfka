...
class Iksemel:
    ...
    def __init__(self):
        self.root = ET.Element("eExact", )
        self.root.set(Iksemel.NS_XSI + "noNamespaceSchemaLocation", "eExact-Schema.xsd")
        self.xmlGLEntries = ET.SubElement(self.root, 'GLEntries')

    def addEntry(self, entry, status, code, data, data_dokumentu, dziennik, typ_dziennika,kwota_brutto):
        self.xmlGLEntry = ET.SubElement(self.xmlGLEntries, 'GLEntry')
        self.xmlGLEntry.set("entry",entry)
        self.xmlGLEntry.set("status",status)
        self.xmlDivision = ET.SubElement(self.xmlGLEntry, "Division")
        self.xmlDivision.set("code", code)
        self.xmlDate = ET.SubElement(self.xmlGLEntry, "Date")
        self.xmlDate.text = data
        self.xmlDocumentDate = ET.SubElement(self.xmlGLEntry, "DocumentDate")
        self.xmlDocumentDate.text = data_dokumentu
        self.xmlJournal = ET.SubElement(self.xmlGLEntry, "Journal")
        self.xmlJournal.set("code", dziennik)
        self.xmlJournal.set("type",typ_dziennika)

        self.xmlGLAccount = ET.SubElement(self.xmlJournal, "GLAccount",
                                     attrib={"code": "   999998", "type": "B", "subtype": "N", "side": "D"})
        self.xmlAmount = ET.SubElement(self.xmlGLEntry, 'Amount')

        self.xmlCurrency = ET.SubElement(self.xmlAmount, "Currency", attrib={"code": "PLN"})
        self.xmlValue = ET.SubElement(self.xmlAmount, "Value")
        self.xmlValue.text = kwota_brutto
...
a = Iksemel()
a.addEntry(entry='6084323', status='E', code='001', data='2013-12-19', data_dokumentu='2013-12-19', dziennik='201', typ_dziennika='I', kwota_brutto='8.73')

print ET.tostring(a.get_xml(), pretty_print=True, xml_declaration=True, encoding='UTF-8')
